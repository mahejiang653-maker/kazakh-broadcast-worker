...
const url='https://kazakh-broadcast-worker.mahejiang653.workers.dev/news-globe-run-20260915-v52-r1.html?v=20260916-v52-visual-12';
...
for(let i=0;i<13;i++){
 ...
 if(i===7){const seq=s.trace.filter(e=>e.type==='diplomacy-country').map(e=>e.iso);if(seq.join('/')!=='KAZ/KOR')throw Error('TOP8 must show other country then event country exactly once: '+seq.join('/'));const loc=s.trace.find(e=>e.type==='two-country-location');if(loc?.eventCountry!=='KOR')throw Error('TOP8 final event country must be KOR');}
