...
const url='https://kazakh-broadcast-worker.mahejiang653.workers.dev/news-globe-run-20260915-v52-r1.html?v=20260916-v52-visual-11';
...
async function waitForLongSequence(i){
 if(i===7)await page.waitForFunction(()=>window.NG14.getV52SceneTrace().filter(e=>e.type==='diplomacy-country').map(e=>e.iso).join('/')==='KOR/KAZ',{timeout:18000});
...
for(let i=0;i<13;i++){
 ...
 if(i===7){const seq=s.trace.filter(e=>e.type==='diplomacy-country').map(e=>e.iso);if(seq.join('/')!=='KOR/KAZ')throw Error('TOP8 must be KOR then KAZ exactly once before location: '+seq.join('/'));}
