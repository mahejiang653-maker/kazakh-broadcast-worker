...
UA='daily-news-image-pack/5.0'; MIN_W=int(os.getenv('MIN_WIDTH','1600')); MIN_H=int(os.getenv('MIN_HEIGHT','900')); FB_W=int(os.getenv('FALLBACK_MIN_WIDTH','1200')); FB_H=int(os.getenv('FALLBACK_MIN_HEIGHT','675')); N=int(os.getenv('IMAGES_PER_NEWS','3')); LIMIT=int(os.getenv('SEARCH_LIMIT','50')); TIMEOUT=35
...
def getbytes(url):
 try:
  r=s.get(str(url or ''),timeout=TIMEOUT,allow_redirects=True); r.raise_for_status()
...
def commons_search(q,item,mw,mh):
 ...
 for p in arr:
  ...
  p['_score']=min(w*h//1000000,20)+(8 if h and 1.4<=w/h<=2.05 else 0)+bonus(b,item)+(4 if any(y in clean(b) for y in ('2026','2025','2024')) else 0); out.append(p)
...
def commons_exact(title,item,mw,mh):
 ...
 for p in arr:
  ...
  if p.get('missing') is not None or w<mw or h<mh or not clic(p): continue
  # Exact curated fallbacks bypass normal relevance matching, but still obey reject/leader/avoid safety.
  b=clean(cblob(p))
  if any(x in b for x in REJECT) or any(x in b for x in LEADERS) or any(clean(x) in b for x in item.get('avoid',[])): continue
  out.append(p)
 return out
...
def save_result(rec,item,data,m,used,total,search_query,quality):
 k=key(m)
 if not k or k in used:return total,False
 fn=f"{int(item['id']):02d}_{len(rec['images'])+1}{ext(m['format'])}"; (IMG/fn).write_bytes(data); m.update({'file':fn,'search_query':search_query,'quality_tier':quality}); used.add(k); rec['images'].append(m); print('   +',fn,m['source'],m['title']); return total+1,True

def main():
 news=json.loads(INPUT.read_text(encoding='utf-8')); shutil.rmtree(OUT,ignore_errors=True); IMG.mkdir(parents=True,exist_ok=True)
 manifest={'generated_at_utc':datetime.now(timezone.utc).isoformat(),'version':'5.0','rules':{'news_count':13,'images_per_news':N,'target_images':13*N,'preferred_minimum':[MIN_W,MIN_H],'fallback_minimum':[FB_W,FB_H],'sources':['Wikimedia Commons','Openverse'],'story_specific_grouped_whitelist':True,'exact_commons_fallbacks':True,'real_images_only':True,'ai_generated_images':False,'reusable_license_required':True},'items':[]}; used=set(); total=0
 for item in news:
  ...
  for quality,mw,mh in [('preferred',MIN_W,MIN_H),('fallback',FB_W,FB_H)]:
   # Curated exact files first: reliable for sparse topics.
   for title in item.get('exact_files',[]):
    ...
    for p in arr:
     r=cdownload(p,mw,mh)
     if r: total,_=save_result(rec,item,r[0],r[1],used,total,'exact:'+title,quality)
   for q in item.get('queries') or []:
    ...
    for source,c in candidates:
     ...
     if r: total,_=save_result(rec,item,r[0],r[1],used,total,q,quality)
    time.sleep(.12)
 ...
 lines=['每日新闻相关图片包 v5','',f'实际下载：{total} 张；目标：{13*N} 张。','采用分组实体白名单 + 人工校验的 Commons 精确文件回退。','来源：Wikimedia Commons + Openverse；仅真实摄影/卫星/设施图。','禁止 AI 图、程序示意图、地图/图表/插画补位。','']+[f"{r['id']:02d}. {r['title']} — {len(r['images'])}/{N} 张" for r in manifest['items']]
