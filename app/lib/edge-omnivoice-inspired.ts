...
function semanticBreak(
  ...
  // Story V14: three breathing levels inside one continuous acoustic state:
  ...
  if (deliveryMode === "story") {
    ...
    if (kind === "paragraph") {
      ...
      // V14 paragraph cadence: make a true paragraph ending clearly audible
    ...
    if (kind === "period") {
      ...
      // V14 completed-sentence breath: keep all declarative sentence endings
...
function naturalTextMarkup(
  ...
  if (deliveryMode === "story") {
    ...
    while ((match = SOFT_SYNTAGMA_PATTERN.exec(text)) && inserted < maxBreaths) {
      ...
      output += `<break time="${clean.length >= 220 ? 40 : 30}ms"/>`;
      cursor = boundary;
