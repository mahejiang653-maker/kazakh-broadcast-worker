(function attach(){
  ...
  function countryScene(n,iso){
    const name=CN[iso]||G.countryNameZh?.(iso)||G.countryName?.(iso)||'相关国家';
    return {...n,sceneMode:'COUNTRY',scenePlan:{...(n.scenePlan||{}),primaryIso3:iso,participants:[],contextCountries:[],adminChain:[],finalLocation:false,regionalContext:false},countryIso3:iso,secondaryCountryIso3:null,secondaryCountry:null,country:name,location:name,region:name+'全境',placeType:'国家',countryOnly:true,noPoint:true,focusLabel:name};
  }
  ...
  G.runSequence=async function(n,iso,s){
    ...
    if(isRemoteDiplomacy){
      // V52 permanent rule: two-country diplomacy is temporal, not simultaneous.
      // Show country A alone, clear it, then country B alone. This prevents flags/names
      // from colliding and makes the narrative order obvious on narrow mobile screens.
      for(const x of participants){
        if(s!==G.navSerial)return false;
        await baseRun(countryScene(n,x),x,s);
        if(s!==G.navSerial)return false;
        await wait(950,s);
      }
      return true;
