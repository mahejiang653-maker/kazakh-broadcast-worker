(function attach(){
  ...
  function isFlagEntity(e){try{if(e?.__v52StoryFlag)return true;const p=e?.billboard?.image;if(!p)return false;let v=p;if(typeof p.getValue==='function')v=p.getValue(C.JulianDate.now());v=String(v?.src||v||'');return /flagcdn\.com|\/flags?\//i.test(v)}catch{return false}}
  function removeEveryFlag(){removeFlags();try{const all=G.viewer.entities.values.slice();for(const e of all)if(isFlagEntity(e))G.viewer.entities.remove(e)}catch{}}
  function center(iso){return G.countries?.get?.(String(iso||'').toUpperCase())?.center||null}
  function addFlag(iso,lon,lat,dx=0){iso=String(iso||'').toUpperCase();const cc=ISO2[iso];if(!cc||!Number.isFinite(+lon)||!Number.isFinite(+lat))return;const e=G.viewer.entities.add({position:C.Cartesian3.fromDegrees(+lon,+lat,90000),billboard:{image:`https://flagcdn.com/w80/${cc}.png`,width:28,height:18,pixelOffset:new C.Cartesian2(dx,-24),disableDepthTestDistance:Number.POSITIVE_INFINITY}});e.__v52StoryFlag=true;flags.push(e)}
  ...
  function showStoryFlags(n,iso){removeEveryFlag();const isos=storyIsos(n,iso);for(let i=0;i<isos.length;i++){const x=isos[i],c=center(x);if(c)addFlag(x,c[0],c[1],i?18:0)}}
  ...
  function cleanOverview(){try{G.navSerial=(G.navSerial||0)+1}catch{}try{G.overviewMode=true}catch{}purgeScene();const ov=document.getElementById('overview');if(ov){ov.classList.remove('show');ov.style.display='none'}try{G.viewer.camera.cancelFlight?.()}catch{}try{G.viewer.camera.flyHome?.(1.1)}catch{}for(const ms of [50,150,400,900,1600,2600])setTimeout(()=>{if(!G.overviewMode)return;purgeScene();const x=document.getElementById('overview');if(x){x.classList.remove('show');x.style.display='none'}},ms)}
  ...
  console.info('[News Globe] V52 r14: all flag entities purged in overview');
})();