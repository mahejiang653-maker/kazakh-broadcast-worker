...
async function final(n,s,flagIso=null){if(!good(n.lon,n.lat))return false;clear();const regional=/海峡|海湾|海域|湾区|群岛|半岛/.test(String(n.focusLabel||n.location||''));if(!await fly(n.lon,n.lat,regional?430000:Math.max(190000,+n.pointHeight||260000),s))return false;G.localLabelEntity=G.label?.(n.focusLabel||n.location,+n.lon,+n.lat,'country');if(G.localLabelEntity)G.localLabelEntity.__v52OwnedLabel=true;if(flagIso)flag(flagIso,+n.lon,+n.lat,25,-11,21,13,'attack');G.__v52Trace.push({stage:'final',lon:+n.lon,lat:+n.lat,label:n.focusLabel||n.location});return wait(2100,s)}async function regional(n,s){const p=n.scenePlan||{},is=[String(p.primaryIso3||n.countryIso3||'').toUpperCase(),...(p.contextCountries||[]).map(x=>String(x).toUpperCase())].filter(Boolean),uniq=[...new Set(is)];clear();let ps=[];for(let i=0;i<uniq.length;i++){const x=uniq[i];if(x==='CHE')await hiSwiss();const color=i===0?'#ff4050':'#3dbdff';territory(x,color);const c=mainland(x);if(c){countryName(x,c[0],c[1],i===0?'#b91f35':'#167caf');flag(x,c[0],c[1],0,-30,32,21,'country')}ps.push(...carts(x))}if(ps.length)await fit(ps,s,900000,6500000);G.__v52Trace.push({stage:'regional',countries:uniq,names:true,flags:true});if(s!==G.navSerial)return false;await wait(1800,s);return final(n,s)}
