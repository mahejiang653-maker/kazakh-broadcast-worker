...
const trace=(stage,n,extra={})=>G.v52AttackSequenceTrace.push({stage,id:+n?.id||0,t:Date.now(),...extra});
...
async function borderStage(att,vic,s,n){const g=closest(att,vic);if(!g)return false;clean();drawCountry(att,COLORS.att);drawCountry(vic,COLORS.vic);trace('border',n,{att,vic,lon:g.mid[0],lat:g.mid[1]});const h=Math.max(700000,Math.min(3600000,650000+g.gap*155000));if(!await flyPoint(g.mid[0],g.mid[1],h,s,.95))return false;return wait(1300,s)}
...
async function finalPoint(n,s){clean();if(!good(n.lon,n.lat))return false;trace('final',n,{lon:+n.lon,lat:+n.lat});if(!await flyPoint(+n.lon,+n.lat,+n.pointHeight||320000,s,1.0))return false;const e=G.viewer.entities.add({position:C.Cartesian3.fromDegrees(+n.lon,+n.lat,30000),point:{pixelSize:10,color:C.Color.fromCssColorString('#ff4050'),outlineColor:C.Color.WHITE,outlineWidth:2,disableDepthTestDistance:Number.POSITIVE_INFINITY}});G.localHighlightEntities?.push(e);G.localLabelEntity=G.label?.(n.focusLabel||n.location,+n.lon,+n.lat,'country');return wait(2600,s)}
async function runAttack(n,iso,s){G.v52AttackSequenceTrace.length=0;const p=n.scenePlan||{},att=String(p.attackerIso3||n.sourceCountryIso3||'').toUpperCase(),vic=String(p.victimIso3||n.targetCountryIso3||n.countryIso3||iso||'').toUpperCase();if(!att||!vic||!good(n.lon,n.lat))return prevRun(n,iso,s);
  if(!await borderStage(att,vic,s,n))return prevRun(n,iso,s);if(s!==G.navSerial)return;
  ...
  clean();trace('route',n,{att,vic,sourceLon:src.lon,sourceLat:src.lat,targetLon:+n.lon,targetLat:+n.lat});if(!await fitRoute(src,{lon:+n.lon,lat:+n.lat},s))return;if(s!==G.navSerial)return;
