...
async function showGeometry(g,label,serial,hold=1500){if(!g)return false;G.clearCountry?.();G.clearLocal?.();const fake={location:label,focusLabel:label,placeType:'地区'};G.drawAdminArea?.(g,fake,120);const p=G.centerOf?.(g);if(p)G.localLabelEntity=G.label?.(label,p[0],p[1],'country');G.flyArea?.({geometry:g},fake,serial,'admin');await wait(hold,serial);if(serial!==G.navSerial)return false;/* V52: subdivision boundary is only a temporary hierarchy cue. Remove it after the subdivision has been shown so it cannot overlap the persistent national border at the final point stage. */G.clearLocal?.();return serial===G.navSerial}
