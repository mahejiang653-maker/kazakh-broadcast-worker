...
G.runSequence=async function(n,iso,s){
 ...
 if(ctx.length){const eventCountry=primary;for(const x of ctx){await runCountry(x,n,s,850,'diplomacy-country');if(s!==G.navSerial)return false;}if(eventCountry){await runCountry(eventCountry,n,s,850,'diplomacy-country');if(s!==G.navSerial)return false;}const q={...n,countryIso3:null,secondaryCountryIso3:null,scenePlan:{...p,primaryIso3:'',participants:[],contextCountries:[]}};trace('two-country-location',{countries:[...ctx,eventCountry].filter(Boolean),eventCountry,location:String(n.focusLabel||n.location||'')});return oldRun?oldRun.call(this,q,'',s):false;}
