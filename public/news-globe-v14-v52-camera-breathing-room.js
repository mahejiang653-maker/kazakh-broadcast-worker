    ...
    try{
      if(options&&options.destination&&options.destination instanceof C.Cartesian3){
        ...
        if(cart&&Number.isFinite(cart.height)&&cart.height>50000){
          ...
          options=Object.assign({},options,{destination:C.Cartesian3.fromRadians(cart.longitude,cart.latitude,h),duration:Math.max(Number(options.duration)||0,1.35),easingFunction:C.EasingFunction.QUADRATIC_IN_OUT});
        }
      }
    }catch(e){}
