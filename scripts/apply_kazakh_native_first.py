...
replacements = [
    ...
    ending: { rateFactor: 0.992, pitchDelta: -0.14, volumeDelta: -0.04 },""", """    title: { rateFactor: 0.994, pitchDelta: -0.015, volumeDelta: 0.11 },
    ...
    volumeDelta: clamp(
      ...
      -0.16,
      0.3,""", """      1 + (role.rateFactor - 1) * importance - climaxLift * 0.001 - endingSettle * 0.003,
      0.982,
      1.008,
    ),
    pitchDelta: clamp(
  ...
  return {
    ...
    volumeDelta: clamp(volumeDelta, -0.2, 0.25),""", """  if (startsWithCue(clean, FOCUS_CUES)) {
    rateFactor *= 0.996;
    volumeDelta += 0.045;
  } else if (startsWithCue(clean, CONTRAST_CUES)) {
    volumeDelta += 0.028;
  ...
  return {
    ...
    volumeDelta: clamp(a.volumeDelta + b.volumeDelta, -0.28, 0.46),""", """    rateFactor: clamp(a.rateFactor * b.rateFactor, 0.96, 1.015),
    ...
    if (next && !hardAfter) items.push({ micro: next.micro, weight: 0.18 });""", """    const items: Array<{ micro: MicroProsody; weight: number }> = [
...
function naturalTextMarkup(text: string) {
  ...
  return escapeXml(text);
    ...
    const tooLong = current.length >= 5;""", """    const tooDifferent = microDistance(currentAverage, phrase.micro) > 2.35;
    const tooLong = current.length >= 8;"""),
]

for old, new in replacements:
    if old not in s:
        raise SystemExit("native-first marker not found:\n" + old[:180])
    s = s.replace(old, new, 1)

path.write_text(s)
print("applied Kazakh native-first phrase prosody")
